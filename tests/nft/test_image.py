...
def test_init_and_find_attributes(random_image_generator):
    assert random_image_generator != None
...
def test_overlay(random_image_generator):
    ...
    for row_idx in range(3):
        ...
        assert pixdata[pix_index + bottom_offset:pix_index + bottom_offset + 3] == lower_left

        if row_idx != 2:
            assert pixdata[pix_index + bottom_offset + 12:pix_index + bottom_offset + 15] == top_lower_right
        else:
            assert pixdata[pix_index + bottom_offset + 12:pix_index + bottom_offset + 15] == bottom_lower_right
